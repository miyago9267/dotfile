...
| `codex-quota.py` | `~/.claude/coralline/codex-quota.py` | Official Codex app-server quota provider |
...
1. copy the renderer, Codex quota provider, wizard, sample input, and bundled themes;
...
python3 -c 'import sys; raise SystemExit(sys.version_info < (3, 11))' || echo "MISSING: Python 3.11+ (REMORA quota only)"
command -v codex || echo "MISSING: authenticated Codex CLI (REMORA quota only)"
```

`jq` is required because coralline uses it at runtime and the installer uses it to merge
`settings.json`. Python 3.11+ and an already authenticated Codex CLI are required only for
REMORA quota fallback; native Claude renders do not start either process. If `jq` is missing,
help the user install it first:
...
For REMORA quota fallback, `codex-quota.py` launches the official `codex app-server --stdio`
protocol with analytics disabled. Codex uses its existing OAuth login internally; Coralline
never asks for credentials or opens a network API. Only normalized 5h/7d values are cached in
`${XDG_CACHE_HOME:-~/.cache}/coralline/codex-quota.json` with private permissions.
...
cp statusline.sh codex-quota.py configure.sh install.sh ~/.claude/coralline/
