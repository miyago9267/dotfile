#!/usr/bin/env bash
# coralline — a configurable, Powerlevel10k-inspired statusline for Codex
...
#   * Everything themeable via ~/.codex/coralline.conf (sourced bash)
#
...
# ── Load user config ─────────────────────────────────────────────────────────
VL_CONF="${CORALLINE_CONFIG:-$HOME/.codex/coralline.conf}"
[ -f "$VL_CONF" ] && . "$VL_CONF"
...
$(printf '%s' "$input" | jq -r '
def model_name:
  if (.model | type) == "object" then (.model.display_name // .model.name // "")
  elif (.model | type) == "string" then .model
  else (.model_slug // env.CODEX_MODEL // "")
  end;
[
  (.workspace.current_dir // .cwd // .current_dir // .workdir // .session.cwd // env.PWD // ""),
  model_name,
  (.context_window.used_percentage // .context.used_percentage // .context.percent_used // .token_usage.context_used_percentage // "" | tostring),
  (.context_window.total_input_tokens // .token_usage.input_tokens // .usage.input_tokens // 0),
  (.context_window.total_output_tokens // .token_usage.output_tokens // .usage.output_tokens // 0),
  (.context_window.current_usage.cache_read_input_tokens // .token_usage.cache_read_input_tokens // .usage.cache_read_input_tokens // 0),
  (.context_window.current_usage.cache_creation_input_tokens // .token_usage.cache_creation_input_tokens // .usage.cache_creation_input_tokens // 0),
  (.rate_limits.five_hour.used_percentage // "" | tostring),
  (.rate_limits.five_hour.resets_at // "" | tostring),
  (.rate_limits.seven_day.used_percentage // "" | tostring),
  (.rate_limits.seven_day.resets_at // "" | tostring),
  (.cost.total_cost_usd // .cost_usd // "" | tostring),
  (.cost.total_lines_added // .lines_added // 0),
  (.cost.total_lines_removed // .lines_removed // 0),
  (.output_style.name // .personality // ""),
  (.cost.total_duration_ms // .duration_ms // .turn.duration_ms // 0)
...
# Lean mode (VL_LEAN=1): no pill backgrounds, segment color applies to text —
# mirrors powerlevel10k's lean style. VL_BG_* values become text colors.
if [ "$VL_LEAN" = "1" ]; then
  VL_CAP_L="" ; VL_CAP_R="" ; VL_SEP=""
  bg() { fg "${1:-}"; return 0; }
  VL_FG_TEXT=""
